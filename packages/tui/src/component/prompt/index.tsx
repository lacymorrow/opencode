...
export function Prompt(props: PromptProps) {
    ...
    bindings: [
      {
        ...
        cmd: async () => {
          const result = await handleShellTabCompletion({
            ...
            cwd: sync.data.path.cwd || sync.data.path.directory || (sync as any).path?.directory || undefined,
          })
          if (result.applied) {
  ...
  const shortenedWorkingDir = createMemo(() => {
    const dir = sync.data.path.cwd || sync.data.path.directory || (sync as any).path?.directory
    if (!dir) return ""
    const home = sync.data.path.home || (sync as any).path?.home || process.env.HOME || process.env.USERPROFILE || ""
      ...
      <box ref={(r: BoxRenderable) => (anchor = r)} visible={props.visible !== false} width="100%">
        ...
        <box width="100%" flexDirection="row" justifyContent="space-between">
          <Switch>
            ...
            <Match when={props.showWorkingDirectory !== false}>
